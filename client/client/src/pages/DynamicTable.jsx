...
import { fetchRowsForTable }   from '../api';
...
const DynamicTableManager = () => {
  ...
  const handleUpdate = async () => {
    try {
      // user types a JSON like:
      // { "filter": { "username":"alice" }, "changes": { "age":30 } }
      const payload = JSON.parse(jsonInput);
  
      if (!payload.filter || !payload.changes) {
        throw new Error('JSON must have “filter” and “changes” objects');
      }
  
      const res = await fetch(
        `/api/tables/${encodeURIComponent(selectedTable)}/rows`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
...
const handleDeleteByFilter = async () => {
    try {
      const payload = JSON.parse(jsonInput); 
      // e.g. { "username": "alice", "active": false }
      ...
      setLog('Row(s) deleted.');
      setEditing(false);
      fetchRows();
    } catch (err) {
      setLog(err instanceof SyntaxError
